...
import { ExternalLink, Trash2, Edit3, Check, X, Calendar, Building2 } from 'lucide-react'
...
export default function ApplicationCard({ app, onStatusChange, onNotesChange, onDelete }: Props) {
  ...
  return (
    <div className="card p-3 space-y-2 group">
      ...
      <select
        ...
        className={`text-xs font-medium rounded-md px-2 py-1 border w-full focus:outline-none focus:ring-1 focus:ring-brand-500 ${cfg.bg} ${cfg.color} ${cfg.border} bg-opacity-50`}
      >
        {ALL_STATUSES.map(s => (
      ...
      {editingNotes ? (
        ...
        <div
          ...
          className="text-xs text-slate-600 hover:text-slate-400 cursor-pointer min-h-[20px] transition-colors flex items-start gap-1"
        >
