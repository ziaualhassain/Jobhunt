  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [email.toLowerCase().trim(), hash, name.trim(), role,
       companyName?.trim() ?? null, companyEmail?.trim() ?? null]
    );
    const user = rows[0];
    const token = jwt.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.status(201).json({
      token,
      user: { ...user, companyName: user.company_name, companyEmail: user.company_email },
    });
  } catch (err) {
...
router.get('/me', requireAuth, async (req, res) => {
  try {
    ...
    const u = rows[0];
    res.json({ ...u, companyName: u.company_name, companyEmail: u.company_email });
  } catch (err) {
